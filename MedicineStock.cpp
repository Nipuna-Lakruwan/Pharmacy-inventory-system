...
public:
	...
	void stockManage(string type)
	{
		...
		if (Medicine::medicineList())
		{
			string id;
			cout << endl
					 << " Enter medicine ID to stock " << type << " : ";
			getline(cin, id);
			...
			else
			{
				...
				while (qty != 1)
				{
					...
					else
					{
						...
						{
							cout << endl
									 << " Stock is currently not enough... Please add stock and try again... Thank!" << endl;
							break;
						}
						else if (qty == 1)
						{
							...
							else
							{
								...
								cout << endl
										 << " Stock for " << stock.medicineDetail.getTitle() << " with id number " << stock.medicineDetail.getId() << " edited!!" << endl;
							}
						}
					}
				}
	...
	void stockRecord_all(string type, int count, char check)
	{
		...
		if (this->stockList(type, sorting))
		{
			...
			if (type == "all")
				...
				cout << "\t\t";
			cout << "\tEnter 2 >> Stock In ";
			if (type == "in")
				this->displaySort(sorting);
			else
				cout << "\t\t";
			cout << "\tEnter 3 >> Stock Out ";
			if (type == "out")
				this->displaySort(sorting);
			else
				cout << "\t\t";
		...
		else
		{
			cout << endl
					 << endl
					 << " Current do not have any stock..." << endl;
			cout << endl
					 << "\t";
			system("pause");
		}
