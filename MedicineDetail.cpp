...
public:
	...
	void displayMedicine(int type)
	{
		...
		if (test == 0)
		{
			...
			cout << setprecision(2) << _purchasePrice << endl;
		...
		if (test == 0)
		{
			...
			cout << setprecision(2) << _soldOutPrice << endl;
		if (type == 1)
		{
	...
	int askPurchasePrice(int action)
	{
		...
		if (action == 3)
		{
			...
			if (!(this->check_double(purchasePrice)))
			{
				...
				_purchasePrice = strtod(purchasePrice.c_str(), &end);
			return 1; // return 1 if the input is valid
		}
		else
		{
	...
	int askSoldOutPrice(int action)
	{
		...
		if (action == 4)
		{
			...
			if (!(this->check_double(soldOutPrice)))
			{
				...
				_soldOutPrice = strtod(soldOutPrice.c_str(), &end);
			return 1; // return 1 if the input is valid
		}
		else
		{
